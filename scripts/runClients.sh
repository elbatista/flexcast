...
do
    java -cp "bin/*:lib/*" MainClient -c $clients -i $ID -d $duration -a $algo -l $locality -w $warehouse -m $msgs $log $tpcc $payload $thinktime $localm -tree $dag_tree >> $basedir/logs/client$ID.txt &
    sleep .1
    ID=$(($ID+1));
done