if [ "$#" -lt 6 ]; then echo "Usage: $0 <duration:sec> <debug:bool> <skeen:type> <tpcc:bool> <#clis> <#servers> <#partitions> <profileron:bool> <#numMsgs> <batchSize> <locality>"; exit 0; fi
i=0; dbg=""; sk=""; tpcc=""; np=""; p=""; nmsgs=""; bs="";locality=""; count=0; ant clean; ant; rm -f -r logs/*  files/*;
...
do
    ...
    for ((i = START; i >= 0; i-=1)) ; do
        java -cp "bin/*:lib/*" MainServer -i $i -d $1 $dbg $sk $p $bs -c $5 >> logs/node$i.txt & sleep .05
    done
