...
if [ "$#" -lt 15 ]; then 
    ...
    <#clis> <#servers> <#nodes> \
    <locality> <#msgs> <#gc(ms)> <#clispernode> <tpcc> <payload> <thinktime> <localm> <dag_tree> <gap: 0 1>"
    exit 0; 
fi
...
clispernode=$9 tpcc=$tpcc payload=$payload thinktime=$thinktime" dag_tree=$dag_tree gap=$gap >> $basedir/logs/execution.log;
echo "------------------------------------------------------------------------------------------------" >> $basedir/logs/execution.log;
...
do
    ...
    echo "$clispernode clients (from id $ID) on $node region $region assume as primary warehouse: $warehouse ($nodewarehouse)" >> $basedir/logs/execution.log;
...
if [ "$gc" -gt 0 ]; then
    ...
    echo started gc client \(id $ID\) on $lastnode >> $basedir/logs/execution.log;
else
...
if [ "$gap" -gt 0 ]; then 
    ...
    do
        warehouse="${warehouses[$nodewarehouse]}"
        echo "gap-client (id $ID) on $node region $region assume as primary warehouse: $warehouse ($nodewarehouse)" >> $basedir/logs/execution.log;
        ./scripts/sshcli.sh $node $basedir $clients $ID $duration $algo $locality $warehouse $msgs $log $tpcc $clispernode $payload $thinktime $localm $dag_tree '-gapcli' # >> $basedir/logs/execution.log;
        sleep 1;
        ((i=$i+1));
        ID=$(($ID+1));

        if [ "$i" -eq 11 ]; then break; fi

    done < <( awk '!/^ *#/ && NF'  "$clifile");

    echo started gap clients >> logs/execution.log
fi
...
expdir="$basedir/experiments/${algodesc[$2]}-dag_tree$dag_tree/${servers}nodes/${3}cli/${locality}%/gc${gc}/lat100/nogap"
